/*
 * Dr.TRonik / YouTube / HAZİRAN 2023 / İzmir / Türkiye 
 * ESP8266, ST7735s, 128x160 1.8" TFT LCD, GYBMEP, Local WebServer...
 * Derleme ve karta yükleme öncesi, tüm kütüphaneler ile kart platformları arduino ide'sine yüklenmiş olmalıdır...
 * YouTube: https://youtu.be/nNVgkpPSD84
 * Bağlantılar: https://github.com/ronaer/ST7735s/blob/main/gybmep_server_bb.jpg
 * st7735: CS_D2, RST_D0, DC_D1, SCL_D5, SDA_D7, VDD_5V, GND_GND, BLK_3.3V
*/
...
//ESP8266 için ST7735 pin bağlantıları - Pin Connections for ESP8266
#define TFT_CS 4    // D2
...
//İnternet üzerinden zamanı alabilme ve işleme - TimeClient
#include <NTPClient.h>
#include <Time.h>
#include <TimeLib.h>
#include "Timezone.h"
...
void setup() {
  ...
  Serial.begin(9600);
  //SPI kütüphanesi ile etkileşim/conflict olmaması için I²C pinlerini seçerken GPIO15(SPI_CS) gibi SPI bağlantılı pinler seçil_ME_meli ...
  Wire.begin(12, 0);  // GPIO 12 / D6 SDA, GPIO 0 / D3 SCL
...
void loop() {
  ...
  server.handleClient();  // istemciden gelen isteklerin işlenebilmesi...
  ...
  if (millis() - last_second > 1000) {
    ...
    //Ekranda IP adresi görünsün istenirse:
    // tft.setCursor(45, 55);
  ...
  // 5 saniyede bir sensörden veri okumak için...
...
// Float olarak sıcaklık değerinin sensörden alınması ve istemci tarafından talep edildiğinde bu değerin web sayfasına gönderilmesi...
void handleTemperature() {
  float t = (bme.readTemperature()) - 1.00;  //PCB üzerinde bir - iki derece kadar ısı farkı oluşabiliyor...
...
void handlePressure() {
  ...
  String Pressure_Value = String(p);
  server.send(200, "text/plane", Pressure_Value);
}
...
void read_data() {
  ...
  char pressure_[5];
  sprintf(pressure_, "%04d", pressure);
